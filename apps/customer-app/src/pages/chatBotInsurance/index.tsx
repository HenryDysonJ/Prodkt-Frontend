import { chatBotAmcStore, useInsuranceDetails, useProductDetails, useUserProfileDetails } from '@core/store';
...
export default function ChatBotInsurance() {
  ...
  const { getInsurancecDetails, standard_qus_and_ans, insurance_id } = useInsuranceDetails();
  ...
  const initialData = async () => {
    getProfileDetails();
    await getInsurancecDetails(product_id || '');
    chatBotHistory({
      id: product_id || '',
      type: 'insurance',
  })
            ...
            return (
              <>
                {/* <CustomChatBot
                  ...
                  isInitialMessage={index === 0 ? true : false}
                  data={val?.content}
                  onClickMessage={initialQuestionsSubmit}
                  header={index === 0 ? "Hi there! Please let me know how can i assist you?" : ""}
                /> */}
                <CustomChatBot
                  url={profileDetails?.profile_image}
                  altText={profileDetails?.first_name}
