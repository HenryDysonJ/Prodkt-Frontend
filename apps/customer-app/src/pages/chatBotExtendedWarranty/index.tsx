import { chatBotAmcStore, useExtendedWarrantyDetails, useUserProfileDetails } from '@core/store';
...
export default function ChatBotExtendedWarranty() {
  ...
  const { getExtendedWarrantyDetails, standard_qus_and_ans, warranty_id } = useExtendedWarrantyDetails();
  ...
  const initialData = async () => {
    getProfileDetails();
    await getExtendedWarrantyDetails(product_id || '');
    chatBotHistory({
      id: product_id || '',
      type: 'ext_warranty',
  })
  ...
  // Amplitude tracking
  useEffect(() => {
    track('Extended warranty chat page', {
      name: 'customer-app',
    });
  }, []);
