...
export interface AmcDetailsCardProps {
  className?: string;
  sx?: SxProps<Theme>;
  ...
  showUpload?: boolean;
  amcUploadComponent?: React.ReactNode;
...
export const AmcDetailsCard = (props: AmcDetailsCardProps): JSX.Element => {
  const {
    product_detail,
    loading,
      ...
      <Stack
        ...
        divider={
          <>
            {product_detail === '' ? (
              <>
                {productDetail?.invoice_details?.[0]?.product_details?.amc === false ? null : (
                  <Divider
                    flexItem
                    sx={{ border: '1.3px dashed', borderColor: 'divider.100' }}
            ...
            {product_detail === undefined ? (
              <Stack direction={'row'} alignItems={'center'} justifyContent={'space-between'}>
                ...
                <Box display={'flex'} gap="10px">
                  <CheckBox checked={isAmcShow === 'Yes'} onClick={() => setIsAmcShow('Yes')} label="Yes" />
                  <CheckBox checked={isAmcShow === 'No'} onClick={() => setIsAmcShow('No')} label="No" />
                </Box>
              </Stack>
            ) : null}
          </Box>
          ...
          <>
            {loading ? (
              ...
              >
                ...
                >
                  <FaceEmojiIcon /> Fetching Amc details...
                </Typography>
              </Box>
            ) : (
              <>
                    ...
                    <Box sx={warrantyDetailsCardStyle.detailsContainer}>
                      ...
                      {showUpload && (
                        <Box sx={warrantyDetailsCardStyle.uploadContainer}>
                          <Typography sx={warrantyDetailsCardStyle.uploadDes}>Upload Amc documents by</Typography>
                          <Box py={1}>{amcUploadComponent}</Box>
